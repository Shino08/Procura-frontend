// ItemDetailsModal.jsx (con permisos: admin edita, usuario solo ve + aprueba)
...
export const ItemDetailsModal = ({
  ...
  // Detectar rol del usuario (asume que guardas el rol en localStorage al login)
  const userRole = localStorage.getItem("rol") || "Usuario"; // o decodifica el JWT
  const isAdmin = userRole === "Administrador";
  const isUser = userRole === "Usuario";

  // Cargar estados (solo si admin)
  useEffect(() => {
  ...
  const handleGuardar = async () => {
    ...
    try {
      ...
      // Crear observación (si hay texto nuevo)
    ...
    <Modal
      ...
      footer={
        <div className="flex flex-col gap-2 sm:flex-row sm:justify-between sm:gap-2">
          {/* Botón "Adjuntar archivos" (ambos roles) */}
          {isAdmin && (
            <button
          ...
          {/* Botón "Aprobar cotización" (solo usuario) */}
          {isUser && (
            <button
    ...
    >
      ...
      {activeTab === "general" ? (
        <div className="space-y-4">
          ...
          {/* Edición de observación (solo admin) */}
          {isAdmin && (
            <div className="rounded-xl bg-blue-50 border border-blue-200 p-4">
          ...
          {/* Vista read-only para usuario */}
          {isUser && item.observacion && (
